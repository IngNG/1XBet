...
int main()
{
    ...
    topMenu[0] = {55,20,180,70,"äèâàíû","Divan" };
    topMenu[1] = {205,20,330,70,"êîéêà", "Krovat"};
    topMenu[2] = {355,20,480,70,"ñòåíû", "Wall"};
    topMenu[3] = {505,20,630,70,"êðåñëî","Chair"};
    topMenu[4] = {655,20,780,70,"êîâðû","Cover"};
    topMenu[5] = {805,20,960,70,"cîõðàíèòü",""};
    topMenu[6] = {1055,20,1180,70,"âûõîä",""};
    ...
    Knopka mainMenu[6];
    mainMenu[0] = {500, 270, 680, 320, "Íà÷àòü"};
    mainMenu[1] = {500, 320, 680, 370, "Ïðîäîëæèòü"};
    mainMenu[2] = {500, 370, 680, 420, "Íàñòðîéêè",  "Íàñòðîéêè íåäîñòóïíû", "Îøèáêà"};
    mainMenu[3] = {500, 420, 680, 470, "Èíôîðìàöèÿ"};
    mainMenu[4] = {500, 470, 680, 520, "Âûéòè"};
    mainMenu[5] = {500, 520, 680, 570, "ìóç_off/on"};
    ...
    while(!exitProgram)
    {
		...
		else if (pageNumber == MENU_INFO)
        {
            ...
            txDrawText(300,100,900,700,
					"èíôîðìàöèÿ\n"
					"ñîçäàòåëè: ×èðàêîâ Äàíèèë \n"
					"Áîðîâêîâ Àíäðåé Âëàäèìèðîâè÷\n"
					"ïðîåêò íàïèñàí ïî ìàòèâàì èãðû\n"
					"Sim Siti\n"
					"òîëüêî â ýòîì ïðîåêòå âû áóäåòå ðåäàêòèðîâàòü\n"
					"êâàðòèðó, à íå ãîðîä\n"
					"ÓÄÀ×ÍÎÃÎ ÐÅÄÀÊÒÈÐÎÂÀÍÈß"
					);
        ...
        else if (pageNumber == MAIN_PAGE)
        {
            ...
            // òóò ðèñóþòñÿ âûáðàííûå êàðòèíêè
            for (int i = 0; i < last_num_obj; i++)
            {
            ...
            for (int i = 0; i < PICT_LEN; i++)
            {
                if (selected_category == pic[i].category and pic[i].knopka())
                {
                    ...
                    if (selected_category == "Wall")
					{
						//const char* text = txInputBox("âûáåðèòå êîë-âî ñòåí");
						//kol_sten = atoi(text);
						vybrana_stena = true;
					}
                }
